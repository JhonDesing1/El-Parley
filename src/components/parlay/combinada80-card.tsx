...
import { ShieldCheck, TrendingUp, Clock, CheckCircle2, XCircle } from "lucide-react";
...
export function Combinada80Card({ parlay }: { parlay: Parlay }) {
  ...
  return (
    <Card className="relative overflow-hidden border-emerald-500/40 bg-gradient-to-br from-emerald-500/8 via-card to-card">
      {/* Glow decorativo */}
      ...
      <div className="relative p-5">
        ...
        {/* Stats */}
        <div className="mb-4 grid grid-cols-3 gap-2 rounded-xl border border-emerald-500/20 bg-background/50 p-3">
        ...
        <div className="space-y-2">
          {parlay.legs.map((leg, idx) => {
            ...
            return (
              <div
                key={leg.id}
                className="flex items-center gap-3 rounded-md border border-emerald-500/15 bg-card/70 px-3 py-2"
              >
                <div className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/15 text-[10px] font-bold text-emerald-400">
                  {idx + 1}
                </div>
      ...
      <CardFooter className="gap-2 pt-0">
        ...
        <Button asChild className="flex-1 border-emerald-500/30 bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30">
